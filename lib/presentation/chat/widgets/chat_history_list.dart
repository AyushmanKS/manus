...
class ChatHistoryListState extends ConsumerState<ChatHistoryList> {
  ...
  void _onScroll() {
    ...
    if (distanceFromBottom <= 40.0 && !_userIsScrolling &&
  ...
  void forceScrollToBottom() {
    ...
    _scrollController
      ...
      if (mounted) _forcingScroll = false;
    });
  }
  ...
  void onScrollMetricsChanged() {
    if (_autoScrollNotifier.value) doubleFrameScrollToBottom();
  }
  ...
  Widget build(final BuildContext context) {
    ...
    ref.listen<List<ChatMessage>>(chatProvider, (final List<ChatMessage>? prev,
        final List<ChatMessage> next,) {
      if (!_autoScrollNotifier.value) return;
      final int nextCount = next.length;
      final int nextLastLen = next.isNotEmpty ? next.last.text.length : 0;
      final bool grew = nextCount > _prevMessageCount ||
          nextLastLen > _prevLastTextLength;
